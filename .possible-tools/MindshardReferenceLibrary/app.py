...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Mindshard Reference Library")
    parser.add_argument("--health", action="store_true", help="Print health JSON and exit")
    parser.add_argument("--no-ui", action="store_true", help="Run without launching the UI")
    args = parser.parse_args(argv)
    runtime = BackendRuntime()
    if args.health:
        print(json.dumps(runtime.get_service("MindshardReferenceLibrary").get_health(), indent=2))
        return 0
